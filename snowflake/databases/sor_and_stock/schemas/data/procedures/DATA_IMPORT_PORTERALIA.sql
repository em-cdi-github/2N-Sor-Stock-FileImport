...
CREATE OR REPLACE PROCEDURE SOR_AND_STOCK{{ sufix }}.DATA.DATA_IMPORT_PORTERALIA("SOURCE_TABLE" VARCHAR, "FINAL_TABLE" VARCHAR, "TYPE" VARCHAR, "PROFILE_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS 'DECLARE
...
BEGIN
    IF (UPPER(TYPE) = $$SOR$$) THEN
        ...
        SELECT 
            ...
            TRY_TO_DATE(FECHA_VENTA)                            AS DATE_OF_SALES,
            "UNNAMED:_2"                                        AS CUSTOMER_NAME,
            "C.I.F."                                            AS CUSTOMER_TAX_ID,
            "C.I.F..1"                                          AS CUSTOMER_VAT,
    ...
    ELSEIF (UPPER(TYPE) = $$STOCK$$) THEN
        ...
        SELECT 
            TO_VARCHAR(INSERT_DATE,$$YYYY-MM$$) AS YEAR_MONTH,
            :PROFILE_NAME                       AS PROFILE,
            "ARTÍCULO"                          AS ITEM_REFERENCE_NUMBER,
            "DESCRIPCIÓN"                       AS PRODUCT_NAME,
