...
CREATE OR REPLACE PROCEDURE SOR_AND_STOCK{{ sufix }}.DATA.DATA_IMPORT_EET("SOURCE_TABLE" VARCHAR, "FINAL_TABLE" VARCHAR, "TYPE" VARCHAR, "PROFILE_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS 'DECLARE
...
BEGIN
    IF (UPPER(TYPE) = $$SOR$$) THEN
        ...
        SELECT 
            ...
            "VAT_NO."                                   AS CUSTOMER_VAT,
            "MANUFACTURER_PART_NO."                     AS ITEM_REFERENCE_NUMBER,
            "DESCRIPTION"                               AS PRODUCT_NAME,
            TRY_TO_DECIMAL(QTY)                         AS QUANTITY,
            PROCESS.F_COUNTRY_CODE("CUST._COUNTRY")     AS COUNTRY_OF_DESTINATION
        FROM 
