...
CREATE OR REPLACE PROCEDURE SOR_AND_STOCK{{ sufix }}.DATA.DATA_IMPORT_GC24_EXPRESS("SOURCE_TABLE" VARCHAR, "FINAL_TABLE" VARCHAR, "TYPE" VARCHAR, "PROFILE_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS 'DECLARE
...
BEGIN
    ...
    ELSEIF (UPPER(TYPE) = $$STOCK$$) THEN
        ...
        SELECT 
            TO_VARCHAR(INSERT_DATE,$$YYYY-MM$$) AS YEAR_MONTH,
            :PROFILE_NAME                       AS PROFILE,
            "LIEF.WERKSART.NR._(GC)"            AS ITEM_REFERENCE_NUMBER,
            BEZEICHNUNG                         AS PRODUCT_NAME,
            TRY_TO_DECIMAL(FACHBESTAND)         AS QUANTITY_OF_STOCK
        FROM 
            IDENTIFIER(:SOURCE_TABLE)
        WHERE 
            QUANTITY_OF_STOCK > 0 ;           
