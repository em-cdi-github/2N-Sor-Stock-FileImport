...
CREATE OR REPLACE PROCEDURE SOR_AND_STOCK{{ sufix }}.DATA.DATA_IMPORT_CONTROL("SOURCE_TABLE" VARCHAR, "FINAL_TABLE" VARCHAR, "TYPE" VARCHAR, "PROFILE_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS 'DECLARE
...
BEGIN
    IF (UPPER(TYPE) = $$SOR$$) THEN
        ...
        SELECT 
            ...
            TRY_TO_DATE("FISCAL_YEAR_/_PERIOD",$$0MM.YYYY$$)    AS DATE_OF_SALES,
            "UNNAMED:_3"                                        AS CUSTOMER_NAME,
            $$NA$$                                              AS CUSTOMER_TAX_ID,
            $$NA$$                                              AS CUSTOMER_VAT,
            MATERIAL                                            AS ITEM_REFERENCE_NUMBER,
            "UNNAMED:_5"                                        AS PRODUCT_NAME,
            TRY_TO_DECIMAL(EA)                                  AS QUANTITY,
            $$NA$$                                              AS COUNTRY_OF_DESTINATION
        FROM 
            IDENTIFIER(:SOURCE_TABLE)
        WHERE 
            QUANTITY > 0
