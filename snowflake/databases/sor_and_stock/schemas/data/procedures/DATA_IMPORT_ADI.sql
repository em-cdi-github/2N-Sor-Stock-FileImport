...
CREATE OR REPLACE PROCEDURE SOR_AND_STOCK{{ sufix }}.DATA.DATA_IMPORT_ADI("SOURCE_TABLE" VARCHAR, "FINAL_TABLE" VARCHAR, "TYPE" VARCHAR, "PROFILE_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS 'DECLARE
...
BEGIN
    IF (UPPER(TYPE) = $$SOR$$) THEN
        ...
        SELECT 
            ...
            TRY_TO_DATE(POST_DATE)                              AS DATE_OF_SALES,
            ...
            PROCESS.F_COUNTRY_CODE(COUNTRY)                     AS COUNTRY_OF_DESTINATION
        FROM                    
            IDENTIFIER(:SOURCE_TABLE)
        WHERE 
            QUANTITY > 0
