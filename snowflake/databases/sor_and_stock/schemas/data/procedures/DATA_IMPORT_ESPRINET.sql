...
CREATE OR REPLACE PROCEDURE SOR_AND_STOCK{{ sufix }}.DATA.DATA_IMPORT_ESPRINET("SOURCE_TABLE" VARCHAR, "FINAL_TABLE" VARCHAR, "TYPE" VARCHAR, "PROFILE_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS 'DECLARE
...
BEGIN
    IF (UPPER(TYPE) = $$SOR$$) THEN
        ...
        SELECT 
            ...
            TRY_TO_DATE(DATE_OF_SALES)                  AS DATE_OF_SALES,
            CUSTOMER_NAME                               AS CUSTOMER_NAME,
            CUSTOMER_TAX_ID                             AS CUSTOMER_TAX_ID,
            CUSTOMER_VAT                                AS CUSTOMER_VAT,
            "2N_REFERENCE_NUMBER_(SKU)"                 AS ITEM_REFERENCE_NUMBER,
    ...
    ELSEIF (UPPER(TYPE) = $$STOCK$$) THEN
        ...
        SELECT 
            TO_VARCHAR(INSERT_DATE,$$YYYY-MM$$) AS YEAR_MONTH,
            :PROFILE_NAME                       AS PROFILE,
            PRODUCT_CODE                        AS ITEM_REFERENCE_NUMBER,
            NAME_OF_THE_PRODUCT                 AS PRODUCT_NAME,
            TRY_TO_DECIMAL(QUANTITY_ON_STOCK)   AS QUANTITY_OF_STOCK
        FROM 
            IDENTIFIER(:SOURCE_TABLE)
        WHERE 
            QUANTITY_OF_STOCK > 0
            AND UPPER(PRODUCT_CODE) != $$GRAND TOTAL$$;
