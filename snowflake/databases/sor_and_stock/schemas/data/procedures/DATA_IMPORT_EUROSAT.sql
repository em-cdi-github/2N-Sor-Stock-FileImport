...
CREATE OR REPLACE PROCEDURE SOR_AND_STOCK{{ sufix }}.DATA.DATA_IMPORT_EUROSAT("SOURCE_TABLE" VARCHAR, "FINAL_TABLE" VARCHAR, "TYPE" VARCHAR, "PROFILE_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS 'DECLARE
...
BEGIN
    IF (UPPER(TYPE) = $$SOR$$) THEN
        ...
        SELECT 
            ...
            TO_DATE(DATUM_PRODEJE,$$DD.MM.YYYY$$)   AS DATE_OF_SALES,
            "ZÁKAZNÍK"                              AS CUSTOMER_NAME,
            "DIČ"                                   AS CUSTOMER_TAX_ID,
            "DIČ"                                   AS CUSTOMER_VAT,
    ...
    ELSEIF (UPPER(TYPE) = $$STOCK$$) THEN
        ...
        SELECT 
            TO_VARCHAR(INSERT_DATE,$$YYYY-MM$$) AS YEAR_MONTH,
            :PROFILE_NAME                       AS PROFILE,
            "DODAVATELSKÝ_KÓD"                  AS ITEM_REFERENCE_NUMBER,
            "SKLADOVÁ_KARTA"                    AS PRODUCT_NAME,
            "MNOŽSTVÍ"                          AS QUANTITY_OF_STOCK
        FROM 
            IDENTIFIER(:SOURCE_TABLE)
        WHERE 
            QUANTITY_OF_STOCK > 0;
